...
def get_realtime_speed(switch):
	...
	lines_list = read_file_1('./results/bwmng.txt')
	first_second = int(lines_list[0][0])
	column_bytes_in_rate = 3   # bytes_in/s
	sw = re.compile(switch)
	...
	for row in lines_list:
		...
		if sw.match(iface_name):
			if (int(row[0]) - first_second) <= 120:   # Take the good values only.
				realtime_speed[int(row[0]) - first_second] += float(row[column_bytes_in_rate]) * 8.0 / (10 ** 6)   # Mbit/s
...
def plot_results():
	...
	fig.set_size_inches(12, 6)
	x = np.arange(0, 121)
	realtime_speed1 = get_realtime_speed('s1-eth1')
	y1 = get_value_list_1(realtime_speed1)
	realtime_speed2 = get_realtime_speed('s1-eth2')
	y2 = get_value_list_1(realtime_speed2)
	realtime_speed3 = get_realtime_speed('s1-eth3')
	...
	fig.set_size_inches(12, 6)
	x = np.arange(0, 121)
	realtime_speed = get_realtime_speed('s1-eth[1-3]')
