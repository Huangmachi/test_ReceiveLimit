...
def traffic_generation(net, topo):
	...
	client1 = net.get('h1')
	client1.cmd("iperf -c %s -t 120 > /dev/null &" % server.IP())   # Its statistics is useless, just throw away.

	time.sleep(30)

	client2 = net.get('h2')
	client2.cmd("iperf -c %s -t 90 > /dev/null &" % server.IP())   # Its statistics is useless, just throw away.

	time.sleep(30)

	client3 = net.get('h3')
	client3.cmd("iperf -c %s -t 60 > /dev/null &" % server.IP())   # Its statistics is useless, just throw away.
